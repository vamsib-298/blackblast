...
- Version 0.5.0 passed all 66 core tests, debug/release APK builds, and both lint tasks. The earlier full 79-test Android sweep was interrupted without a final host report. That full-suite result remains UNVERIFIED until a complete rerun is captured.
- Resumption on a freshly booted read-only API 36 emulator produced two pre-test startup ANRs: [full-suite attempt](../../artifacts/test-results/android-20260908-193537.txt) and [three-test Activity control](../../artifacts/test-results/android-20260908-193815.txt). Both ran zero tests; Android exit records identify PIDs 2710 and 4300 as failing to complete startup. The unchanged app launched normally in 3301ms. Root cause remains UNKNOWN; no app fix is claimed.
- Before the third attempt, Android's supported `cmd package compile -m speed -f -v PACKAGE` was run for the app and test APKs only. Android selected the `verify` filter and generated verification caches. No app data, test assertions, test count, or ANR timeout was changed. The full 79-test sweep was then restarted; cache preparation is a recorded test condition, not a proven ANR fix.
